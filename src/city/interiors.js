...
// Placed off the penthouse rather than picked from the registry: it is the
// same building, at street level. See _placeHomeLobby.
const HOME_LOBBY = {
...
export class Interiors {
  ...
  async load(url = '/models/manhattan/interiors.glb',
    graphUrl = '/models/manhattan/streets/street_graph.json') {
  ...
  /**
   * The ground floor of the building the penthouse is in.
   *
   * The penthouse is the one anchored room the street cannot reach: it is a
   * couple of hundred metres up, and doors.json cuts an opening at bid 20263
   * for `home_lobby`, not for the penthouse. Without this room that entry
   * resolves to nothing and the building stays sealed — which is how it
   * behaved before, because this lobby was defined inside the authoring app's
   * scripted corridor tour rather than with the other rooms. It is a room, so
   * it lives here; the tour did not come across.
   */
  _placeHomeLobby() {
    const pent = this.rooms.find((r) => r.key === 'penthouse')
    if (!pent || !pent.building || !this.src) return null
    if (this.rooms.some((r) => r.key === HOME_LOBBY.key)) return null
    const b = pent.building
    // Same building, same frontage, street level.
    const lobby = this.place(HOME_LOBBY, this.src,
      new THREE.Vector3(b.x, this.groundY, -b.y), pent.yaw,
      { building: b, atStreet: true })
    if (!lobby) return null
    // The lift is a two-way threshold, so the penthouse gains the return leg.
    pent.links = [...(pent.links || []), {
      to: HOME_LOBBY.key, label: 'Lift — lobby',
