...
export class TileStreamer {
  ...
  constructor(scene, meta, material = null, layer = 'world') {
    ...
    // parks stay on the plain one. They are shared across every tile, so
    // tile-level disposal must skip them (isSharedCityMaterial).
    this.plainMaterial = new THREE.MeshLambertMaterial({
      vertexColors: true,
      color: 0xffffff,
    })
    this.fallbackMaterial = new THREE.MeshLambertMaterial({ color: 0x8a8f96 })
    this.plainMaterial.userData.cityShared = true
    this.fallbackMaterial.userData.cityShared = true
