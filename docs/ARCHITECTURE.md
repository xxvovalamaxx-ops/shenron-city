...
The current build is a standalone game. It has no runtime connection to
Mission Control or the host computer. The goal of this phase is to make the
world, movement, interaction, doors, elevator, residents, and user experience
solid before any external integration returns.
...
├── agents/       resident visuals and deterministic dialogue
...
- One playable route and one detailed floor.
- Procedural geometry only; no production asset pipeline yet.
- Fictional residents are abstract forms with deterministic dialogue.
