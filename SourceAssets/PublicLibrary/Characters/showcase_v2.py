"""
Character Library Showcase v6 — v3's working positioning + recursive mesh collection.
"""
import bpy
import os
import math
...
def get_all_meshes(objs):
    """Recursively find all MESH objects under imported objects."""
    meshes = []
    visited = set()
    stack = list(objs)
    while stack:
...
# ── Import each character — v3's working approach ────────────────────
...
for idx, (name, fp) in enumerate(characters):
    ...
    # Apply location on each object individually (bakes location into vertex data)
    all_meshes = get_all_meshes(objs)
    for o in all_meshes:
    ...
    # Now read vertex positions (in world space after apply)
    ...
    for mesh_obj in all_meshes:
        if mesh_obj.data and mesh_obj.data.vertices:
            for v in mesh_obj.data.vertices:
                co = v.co  # already in world space after transform_apply
    ...
    if min_z == float('inf'):
        print(f"  SKIP {name}: no mesh vertices")
        continue
    ...
    # Position in grid (v3's exact working formula)
    for o in objs:
...
# ── Camera — orthographic (v3's exact working setup) ─────────────────
cam_x = gcx
cam_y = gcy - 30
cam_z = grid_h / 2 + 2
