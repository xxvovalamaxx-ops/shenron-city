"""
Character Library Showcase v7 — depsgraph bounds, root-based positioning.
No transform_apply. Uses depsgraph for accurate world-space bounds.
"""
import bpy
import os
import math
...
def find_root(objs):
    """Find the root object (armature if present, else first mesh)."""
    for o in objs:
        if o.type == 'ARMATURE':
            return o
    for o in objs:
...
def get_world_bbox(objs, depsgraph):
    """Get world-space bounding box using depsgraph (handles armature deformation)."""
...
# ── Import and place each character ──────────────────────────────────
...
for idx, (name, fp) in enumerate(characters):
    ...
    # Compute where root origin currently is in world space
    root_world = root.matrix_world.translation.copy()

    # Current character center relative to root origin:
    # char_center_world = root_world + char_offset
    # char_offset = cx - root_world.x (in x), etc.
    char_offset_x = cx - root_world.x
    char_offset_y = cy - root_world.y
    char_offset_z = bottom - root_world.z

    # After scaling by s around root, character offset becomes s * char_offset
    # Final char center = new_root_world + s * char_offset
    # We want final char center.x = gx, so:
    # new_root_world.x + s * char_offset_x = gx
    # new_root_world.x = gx - s * char_offset_x
    new_root_x = gx - s * char_offset_x
    new_root_y = gy - s * char_offset_y
    # Bottom should be at z=0:
    # new_root_world.z + s * char_offset_z = 0
    new_root_z = -s * char_offset_z

    root.location = (new_root_x, new_root_y, new_root_z)
    root.scale = (s, s, s)
