"""
Character Library Showcase v5 — v3's working camera + proper world-space bounds.
"""
import bpy
import os
import math
...
def collect_meshes(objs):
    """Recursively collect all MESH objects from objects and their children."""
    meshes = []
    visited = set()
    def _walk(o):
        if id(o) in visited:
...
def world_bbox_meshes(meshes):
    """Get world-space bounding box for mesh objects using evaluated depsgraph."""
    mn = Vector((float('inf'), float('inf'), float('inf')))
    mx = Vector((float('-inf'), float('-inf'), float('-inf')))
    found = False
    for o in meshes:
        if not o.data or not o.data.polygons:
            continue
        mat = o.matrix_world
...
for idx, (name, fp) in enumerate(characters):
    ...
    meshes = collect_meshes(objs)
    if not meshes:
        print(f"  SKIP {name}: no meshes")
        continue

    mn, mx = world_bbox_meshes(meshes)
    if mn is None:
        print(f"  SKIP {name}: no bbox")
        continue
    ...
    # Move all objects relative: center at grid position, ground at z=0
    for o in objs:
        # Offset = grid_pos - (current_center * scale) - current_location
        # We want final world center = (gx, gy) and bottom at z=0
        # Current center is (cx, cy, bottom) in world space
        # After scale s around origin, center moves to (cx*s, cy*s, bottom*s)
        # We want it at (gx, gy, 0)
        dx = gx - cx * s - o.location.x * (1 - s)
        dy = gy - cy * s - o.location.y * (1 - s)
        dz = -bottom * s - o.location.z * (1 - s)
        o.location.x += dx
        o.location.y += dy
        o.location.z += dz
        o.scale = (s, s, s)
...
# ── Camera — orthographic (same as v3 which worked) ──────────────────
cam_x = gcx
cam_y = gcy - 30
cam_z = grid_h / 2 + 2
